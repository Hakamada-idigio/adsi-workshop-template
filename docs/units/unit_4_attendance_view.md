...
- **US-004**: 社員として、承認前の打刻（出退勤・休憩とも）を自分で修正したい
- **US-006**: 社員として、自分の日別・月別の勤務実績を一覧で確認したい
- **US-007**: 社員として、フレックスのコアタイム（10:00〜15:00）・総労働時間の過不足を把握したい
...
| GET | /api/attendance/monthly?yearMonth={YYYY-MM}&page=0&size=31 | 月別勤怠一覧（ページネーション） |
| PUT | /api/attendance/{id} | 打刻修正（出退勤・休憩とも。承認前のみ直接修正） |
...
**月別勤怠一覧:**
1. 指定年月の AttendanceRecord 一覧を取得（ページネーション対応）
2. 各日の実労働時間を計算
3. 月合計実労働時間・所定労働時間（7.25h × 営業日数）・過不足を計算して返却
4. コアタイム情報（10:00〜15:00）を含める
